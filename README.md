...
Code here acts as a ftmiddle between LoRa gateways running Semtech packet forwarders and servers ingesting packet forwarder data.  For you non-geeks, what this code does is tell your miner to report different signals to the blockchain than what the packet forwarder is actually receiving.  
...
Clone this repository onto the machine running your miner. 
    ...
    git clone https://github.com/farukest/ftmiddle.git
...
Now install Ftmiddle in its own working directory. Go to /ftmiddle and
...
#### Ftmiddle Settings (on the miner)

In the directory /ftmiddle 
...
Great, so now you've told Ftmiddle where to "listen" when it comes to the miner.  

Next, make the ftmiddle.conf file in /ftmiddle and tell it what to do with what it hears from the packet forwarder. 

`sudo nano ftmiddle.conf`. 

Then enter your arguments.  
Example 1 (for a Nearson 9 db antenna where you'd want to drop the rx receipts by 9)  
`ftmiddle_args="--rx-adjust -9" `. 
  
Example 2 (an antenna over 13 db where you need to drop the tx by 4 in order to not break FSPL and adjust the RX by 13 to keep within RSSI boundaries) `ftmiddle_args="--tx-adjust -4 --rx-adjust -13"`. 

Next, we'll enable Ftmiddle on the miner:

`sudo systemctl enable ftmiddle`. 
...
#### On the Gateway
Now, on the gateway you're going to change the UDP port from the default (1680) to that of Ftmiddle (1681).  This tells the gateway to talk to Ftmiddle instead of the miner.  You're also going to point your gateway to your miner.
...
Now your gateway is pointing to Ftmiddle (1681) instead of to your miner (1680).  
...
Now that the gateway is directed to your miner, you'll need to start Ftmiddle on the miner, so:    

#### Back on your Miner:

`sudo systemctl start ftmiddle`. 

Check your work with:  

`systemctl status ftmiddle`

and

`sudo journalctl -u ftmiddle`

this mirrors the output of the ftmiddle.log file and you can use the command `q` to quit it.

#### Details & What the Startup Scripts Do

The startup scripts will check for a text
file named `/home/ftmiddle/ftmiddle.conf`, which can contain the following
settings.

* `ftmiddle_config_dir`

  The directory in which `ftmiddle` should search for its
  upstream gateway configurations.  (See the **Configuration files for
  ftmiddle** section).

  Default: `/home/ftmiddle/configs`

* `ftmiddle_python`

  The python interpreter to use when starting `ftmiddle` if you need
...
* `ftmiddle_port`
  ...
  The UDP port on which `ftmiddle` should listen for incoming packets
  from gateways.
...
* `ftmiddle_args`
  ...
  Additional arguments to pass to ftmiddle when running. Make sure to
  use double quotes when setting this variable.
  
### Docker Container Setup
Examples:
```bash
docker run --name diyftmiddle -P -td farukest/ftmiddle:latest      
```
```bash
docker run \
    --net='bridge' \
    -p 1681:1681/udp \
    -e ftmiddle_port=1681 \
    -e ftmiddle_tx_adjust='--tx-adjust 0' \
    -e ftmiddle_rx_adjust='--rx-adjust 0' \
    -e gateway_ID=AA555A0000000000 \
    -e server_address=localhost \
    -e serv_port_up=1680 \
    -e serv_port_down=1680 \
    --name diyftmiddle -P -td farukest/ftmiddle:latest
```    
## How It Works
